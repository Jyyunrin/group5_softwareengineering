...
export default function MultiCheckboxStep({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!allowEmpty) {
      const msg = validate ? validate(clean) : clean.length ? null : "Please select at least one option.";
      if (msg) {
        setError(msg);
        return;
      }
          ...
          return (
            <label
              ...
              className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer border transition-all select-none ${
                checked ? "bg-blue-100 border-blue-400" : "bg-white border-gray-200 hover:bg-gray-50"
              }`}
            >
              <input
                type="checkbox"
